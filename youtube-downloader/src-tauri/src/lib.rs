...
#[tauri::command]
async fn search_videos(query: String) -> Result<Vec<VideoItem>, String> {
    let yt = find_ytdlp();
...
#[tauri::command]
async fn get_video_info(url: String) -> Result<VideoItem, String> {
    let yt = find_ytdlp();
    let url2 = url.clone();
    let url3 = url.clone();
    tokio::task::spawn_blocking(move || {
...
#[tauri::command]
async fn get_playlist(url: String) -> Result<Vec<VideoItem>, String> {
    let yt = find_ytdlp();
    tokio::task::spawn_blocking(move || {
        let output = Command::new(yt)
